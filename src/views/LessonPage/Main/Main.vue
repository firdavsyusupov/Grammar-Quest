    ...
    <div class="container">
      ...
      <div class="con">
        <div class="cards">
          ...
          >
            {{ card.label }}
            <!-- Displaying the generated label -->
          </div>
        </div>

        <!-- Display Questions for the selected card -->
...
const selectedCard = ref(null); // Initialize selectedCard here
...
const generateLabel = (level, index) => {
  ...
  for (let i = 0; i < levelIndex; i++) {
    totalCardsBeforeLevel += cardsData.filter(card => card.level === levels[i]).length;
  }
...
const goToQuestionsPage = (id) => {
  if (cardsWithActiveState.value.find((card) => card.id === id)?.isActive) {
    ...
    selectedCard.value = cardsWithActiveState.value.find((card) => card.id === id);
